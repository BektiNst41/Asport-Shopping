                ...
                <div class="container mx-auto items-center p-5">
                    <ul class="flex items-center space-x-4 ">
                        ...
                        <li class="relative group">
                            <a href="men.php" class="font-semibold text-black px-3 py-2  hover:bg-black hover:text-white active:bg-blue-500 transition-all duration-150 ease-in-out">Men</a>
                        ...
                        <li class="relative group">
                            <a href="women.php"
                                class="border-b-2 border-black font-semibold text-black px-3 py-2  hover:bg-black hover:text-white transition-all duration-150 ease-in-out">Women</a>
                            <ul class="absolute -right-10 hidden bg-white text-black shadow-lg mt-2 w-44 z-10 group-hover:block">
                                <?php
                                    $tampilKategori = "SELECT * FROM kategori ORDER BY id_kategori DESC";
                                    $resultKategori = mysqli_query($conn, $tampilKategori);
...
<div class="bg-white text-black w-60 h-screen fixed">
        ...
        <div class="text-2xl ml-10 font-bold mb-10 mt-28">Women Shoes</div>
        <!-- Menu Sidebar -->
        <ul class="">
        <?php
        $tampilKategori = "SELECT * FROM kategori ORDER BY id_kategori DESC";
    ...
    <?php
        if (isset($_GET['kategori_id'])) {
            ...
            $query = "SELECT produk.*, jenis.nama_jenis, kategori.nama_kategori FROM produk
                      ...
                      WHERE jenis.nama_jenis = 'women' AND kategori.id_kategori = $kategori_id";
        } else {
            $query = "SELECT produk.*, jenis.nama_jenis, kategori.nama_kategori FROM produk
                      INNER JOIN jenis ON produk.id_jenis = jenis.id_jenis
                      INNER JOIN kategori ON produk.id_kategori = kategori.id_kategori
                      WHERE jenis.nama_jenis = 'women'AND stok_produk > 0 ORDER BY id_produk DESC";
        } 
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            <a href="src/homeUSER/desc.php?id_produk=<?= $data["id_produk"];?>">
                ...
                <div class="">
                    ...
                    <p class="font-normal text-gray-700 "><?php echo $data['nama_jenis'] ?> | <?php echo $data['nama_kategori'] ?></p>
                ...
                <div class="flex justify-between mt-4">
                    <div class="mb-2">
                        ...
                        <a href="buy.php?id_produk=<?= $data["id_produk"];?>" class="bg-black py-2 px-6 text-sm rounded-md text-white hover:bg-gray-600">BUY</a>
                    </div>           
            </div>
            </form> 
            <?php 
        ...
        <script>
            ...
            keyword.addEventListener('keyup', function() {
                ...
                xhr.open('GET', 'ajax/women.php?keyword=' + keyword.value, true);
                xhr.send();
            });
        </script>
